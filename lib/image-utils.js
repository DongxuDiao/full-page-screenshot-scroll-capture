...
const ImageUtils = (() => {
   ...
   * @param {number} totalCssHeight - Optional final page height in CSS pixels
   * @returns {Promise<{canvas: OffscreenCanvas, width: number, height: number}>}
   */
  async function stitchFrames(frames, viewportHeight, overlap = 200, dpr = 1, totalCssHeight = 0) {
    ...
    for (const frame of frames) {
      ...
      images.push(await cropFrameIfNeeded(img, frame.captureRect, dpr));
    }
    ...
    // Calculate total height. Prefer the real document height when the caller
    // knows it; otherwise fall back to the stitched frame estimate.
    const estimatedHeightPx = Math.round(frames.length * frameHeight -
      (frames.length - 1) * overlapPx);
    const totalHeightPx = totalCssHeight
      ? Math.min(Math.round(totalCssHeight * dpr), estimatedHeightPx)
      : estimatedHeightPx;
    ...
    if (totalCssHeight && frames.every((frame) => typeof frame.scrollY === 'number')) {
      for (let i = 1; i < images.length; i++) {
        const y = clamp(
          Math.round(frames[i].scrollY * dpr),
          0,
          Math.max(0, totalHeightPx - frameHeight)
        );
  ...
  function cropFrameIfNeeded(img, rect, dpr) {
    if (!rect) return img;
  ...
  async function toDataUrl(canvas, format = 'image/png', quality = 0.92) {
    ...
    return `data:${blob.type || format};base64,${btoa(binary)}`;
  }
  ...
  async function loadImage(dataUrl) {
    ...
    throw new Error('No image decoder is available in this context');
  }
